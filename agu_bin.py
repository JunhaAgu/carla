...
def write_bin(filename, field_list, field_names):
  """
  Write ".bin" files
...
def write_label(filename, field_list, field_names):
  """
  Write ".label" files
  ...
  for i, field in enumerate(field_list):
    if field is None:
      print('WRITE_LABEL ERROR: a field is None')
      return False
    elif field.ndim > 2:
      print('WRITE_LABEL ERROR: a field have more than 2 dimensions')
      return False
    elif field.ndim < 2:
      field_list[i] = field.reshape(-1, 1)
  ...
  with open(filename, 'ab') as binfile:
    ...
    #     type_list += [(field_names[i], field.dtype.str)]
    #     i += 1
    #     # [('instance', '<u4'), ('semantic', '<u4')]
    type_list = [('label', '<u4')]
    new_field_names = ['label']
    data = np.empty(field_list[0].shape[0], dtype=type_list)
    i = 0
    for fields in field_list:
      # for field in fields.T:
      shifted_inst_label = fields.T[0]<<16
      data[new_field_names[i]] = shifted_inst_label+field.T[1]
      i += 1
